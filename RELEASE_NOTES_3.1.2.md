...
Latest stable release. Supersedes 3.1.0 and 3.1.1.
...
- All REST endpoints return `503 Service Unavailable` during active OTA download (except `GET /api/v2/ota` for progress monitoring)
...
- REST API response serialization catches `std::bad_alloc` and returns 503 instead of crashing
- REST API blocked during OTA download to prevent heap exhaustion from concurrent pollers (HA integration, MQTT scripts, browser tabs)
