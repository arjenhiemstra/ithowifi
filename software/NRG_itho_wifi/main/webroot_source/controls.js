...
<div id="main" style="max-width: 1600px;">
    ...
    <label for="wiz-sim-devtype">Simulate device type: </label>
    <select id="wiz-sim-devtype">
      <option value="">(actual device)</option>
      <option value="CVE">CVE</option>
      <option value="CVE-Silent">CVE-Silent</option>
      <option value="CVE-S Eco2">CVE-S Eco2</option>
      <option value="CVE-SilentExtPlus">CVE-SilentExtPlus (HRU200)</option>
      <option value="HRU 250-300">HRU 250-300</option>
      <option value="HRU 350">HRU 350</option>
      <option value="HRU ECO-fan">HRU ECO-fan</option>
      <option value="Heatpump">Heatpump (WPU)</option>
    </select><br><br>
    <button id="button-wizardreactivate" type="button" class="pure-button pure-button-primary">Reactivate wizard</button>

    <fieldset>
        <legend><br>Live device simulation (Simulated Device Runtime):</legend>
        <p>When enabled, the firmware behaves as if the selected Itho device is attached: the web interface, REST API
            and MQTT serve realistic simulated telemetry and accept commands, without any Itho unit connected.<br>
            Scenario changes apply immediately; enable/profile/seed changes require a reboot.</p>
        <span>Status: </span><span id="sim_status">unknown</span><br><br>
        <label for="sim-enable">Enable simulation: </label><input id="sim-enable" type="checkbox"><br><br>
        <label for="sim-profile">Device profile: </label>
        <select id="sim-profile"></select><br><br>
        <label for="sim-scenario">Scenario: </label>
        <select id="sim-scenario"></select><br><br>
        <label for="sim-seed">Telemetry seed: </label>
        <input id="sim-seed" type="number" min="0" max="4294967295" size="12" value="12345"><br><br>
        <button id="button-sim-apply" class="pure-button pure-button-primary">Apply</button>
        <button id="button-sim-applyreboot" class="pure-button pure-button-primary">Apply &amp; reboot</button><br><br>
        <span id="sim_result"></span>
    </fieldset><br>

    <p>JavaScript console debug:</p>
    <button id="jsdebug" type="button" class="pure-button pure-button-primary">Toggle</button>
    ...
    <form class="pure-form pure-form-aligned">
        <fieldset>
            ...
            <button id="rfdebug-12761" type="button" class="pure-button pure-button-primary">Send 31D9</button>&nbsp;speed:
            ...
            <br><br>
            <button id="rfdebug-12762" type="button" class="pure-button pure-button-primary">Send 31DA</button>&nbsp;faninfo:
            ...
            <fieldset>
                ...
                <p>Only use this feature (briefly) for debugging issues. This option enables an extra menu where more I2C options and logging is available. <br>Enabling the
                    I2C
                    debug menu will also halt I2C bus activity in case of errors to be able to save logging!</p>
                <button id="button-i2cdebugon" type="button" class="pure-button pure-button-primary">Debug on</button>
                <button id="button-i2cdebugoff" type="button" class="pure-button pure-button-primary">Debug off</button>
                <br>
                ...
                <button id="button-i2csnifferon" type="button" class="pure-button pure-button-primary">Sniffer on</button>
                <button id="button-i2csnifferoff" type="button" class="pure-button pure-button-primary">Sniffer off</button>
                <br><br>
                <legend><br>Low level Itho I2C commands:</legend><br>
                <button id="ithobutton-type" type="button" class="pure-button pure-button-primary">Query Devicetype</button><br>
                <span>Result:&nbsp;</span><span id="ithotype"></span><br><br>
                <button id="ithobutton-statusformat" type="button" class="pure-button pure-button-primary">Query Status
                    Format</button><br><span>Result:&nbsp;</span><span id="ithostatusformat"></span><br><br>
                <button id="ithobutton-status" type="button" class="pure-button pure-button-primary">Query
                    Status</button><br><span>Result:&nbsp;</span><span id="ithostatus"></span><br><br>
                <button id="button2410" type="button" class="pure-button pure-button-primary">Query 2410</button>setting index: <input
                ...
                <button id="button2410set" type="button" class="pure-button pure-button-primary">Set 2410</button>setting index:
                ...
                <button id="ithobutton-31DA" type="button" class="pure-button pure-button-primary">Query
                    31DA</button><br><span>Result:&nbsp;</span>
                <span id="itho31DA"></span><br><br><button id="ithobutton-31D9"
                    type="button" class="pure-button pure-button-primary">Query 31D9</button><br><span>Result:&nbsp;</span><span
                    id="itho31D9"></span><br><br>

                <button id="ithobutton-10D0" type="button" class="pure-button pure-button-primary">Filter
                ...
                <button id="buttonC000" type="button" class="pure-button pure-button-primary">Send CO2 speed</button>
                ...
                <button id="button9298" type="button" class="pure-button pure-button-primary">Send CO2 value</button>
                Value: <input id="itho_9298_val" type="number" min="0" max="65535" size="4" value="0"><br><br>
                <button id="button4210" type="button" class="pure-button pure-button-primary">Query
                    Counters</button><br><span>Result:&nbsp;</span><span id="ithocounters"></span><br><br>
                <button id="buttonCE30" type="button" class="pure-button pure-button-primary">Send CE30</button>
                ...
                <button id="button4030" type="button" class="pure-button pure-button-primary">Set 4030 Manual Control</button>
...
<script>
    ...
    $id("button-wizardreactivate").addEventListener('click', function () {
        if (confirm("This will reactivate the setup wizard. Are you sure?")) {
            var simSel = $id('wiz-sim-devtype');
            wizardSimDevType = (simSel && simSel.value) ? simSel.value : null;
            update_page('wizard');
            initWizard(1);
        }
    });
    ...
    function simRefresh() {
        fetch('/api/v2/simulation').then(function (r) { return r.json(); }).then(function (j) {
            var s = j.data && j.data.simulation;
            if (!s) return;
            var prof = $id('sim-profile'), scen = $id('sim-scenario');
            prof.innerHTML = '';
            (s.available_profiles || []).forEach(function (n, i) {
                var o = document.createElement('option'); o.value = i; o.textContent = n; prof.appendChild(o);
            });
            scen.innerHTML = '';
            (s.available_scenarios || []).forEach(function (n, i) {
                var o = document.createElement('option'); o.value = i; o.textContent = n; scen.appendChild(o);
            });
            $id('sim-enable').checked = !!s.enabled;
            prof.value = s.profile;
            var scIdx = (s.available_scenarios || []).indexOf(s.scenario);
            if (scIdx >= 0) scen.value = scIdx;
            $id('sim-seed').value = s.seed;
            $id('sim_status').textContent = s.active
                ? ('running - ' + s.profile_name + ', scenario ' + s.scenario + (s.reboot_required ? ' (reboot required for pending changes)' : ''))
                : (s.enabled ? 'enabled - reboot required' : 'disabled');
        }).catch(function () { $id('sim_status').textContent = 'unavailable'; });
    }
    function simApply(reboot) {
        var body = {
            enable: $id('sim-enable').checked,
            profile: parseInt($id('sim-profile').value || '0', 10),
            scenario: parseInt($id('sim-scenario').value || '0', 10),
            seed: parseInt($id('sim-seed').value || '0', 10)
        };
        fetch('/api/v2/simulation', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
            .then(function (r) { return r.json(); }).then(function (j) {
                var d = j.data || {};
                if (reboot) {
                    $id('sim_result').textContent = 'settings saved - rebooting...';
                    websock_send('{"reboot":true}');
                } else {
                    $id('sim_result').textContent = (d.result || 'saved') + (d.reboot_required ? ' - reboot required' : '');
                    simRefresh();
                }
            }).catch(function () { $id('sim_result').textContent = 'request failed'; });
    }
    $id('button-sim-apply').addEventListener('click', function (e) { e.preventDefault(); simApply(false); });
    $id('button-sim-applyreboot').addEventListener('click', function (e) {
        e.preventDefault();
        if (confirm('Apply simulation settings and reboot?')) simApply(true);
    });
    simRefresh();
</script>
`;
...
<script>
  ...
  if (itho_control_interface === 1) {
    ...
    // RF command buttons -> use the configured RF CO2 send remote (read at click time)
    ['low','auto','high','timer1','timer2','timer3'].forEach(function(cmd) {
      var btn = $id('rfcmd-' + cmd);
      if (btn) btn.onclick = function() {
        var co2idx = parseInt(localStorage.getItem('itho_rf_co2_remote_idx') || '0');
        websock_send('{"remote":' + co2idx + ', "command":"' + cmd + '"}');
      };
    });
...
<div class="wizard-step" id="wizard-step-2">
  ...
  <div style="margin: 1em 0;">
    <button id="wiz-override-defaults" type="button" class="pure-button" onclick="wizardOverrideDefaults()">
      <span id="wiz-override-icon">🔒</span> Override Defaults
    </button>
      ...
      <div id="wiz-vrem-section">
        ...
        <div class="wizard-info-box">
          <p>A virtual remote lets the add-on emulate a physical remote to directly control the Itho unit.
            ...
            Most fan devices benefit from at least one virtual remote for control.</p>
        </div>
        <div class="pure-control-group">
          <label for="itho_numvrem">Number of virtual remotes</label>
        ...
        <div class="pure-control-group" id="wiz-vremtype-row">
          ...
          <select id="wiz-vremtype">
            ...
            <option value="0x22F8">RFT DF/QF (DemandFlow/QualityFlow)</option>
          </select>
        </div>
      </div>
...
<div class="wizard-step" id="wizard-step-3">
  ...
  <div class="wizard-info-box">
    <p>RF remotes are wireless remotes emulated by the add-on using the CC1101 RF module. By pairing the add-on as an
      ...
      Itho unit.</p>
    <p>Setting the "Remote function" to "Receive" ensures commands from a physical remote will be received, processed,
      and translated to the proper commands for your Itho unit by the add-on. Make sure the remote is unpaired from your Itho to prevent command conflicts.</p>
    <p>If you set the "Remote function" to "Monitor only", commands are received by the add-on and relevant information is
      published on the APIs and web interface. No action is taken by the add-on to control the Itho; leave the remote paired with the
      Itho unit in that case.</p>
  ...
  <form class="pure-form pure-form-aligned">
    <fieldset>
      ...
      <div class="pure-control-group" id="wiz-numrfrem-row">
        <label for="itho_numrfrem">Number of RF remotes</label>
        <input id="itho_numrfrem" type="number" min="1" max="12" size="6">
      </div>
      <div class="pure-control-group" id="wiz-rfremtype-row">
    ...
    <fieldset>
      ...
      <div class="pure-control-group" id="wiz-rf-buttons">
      ...
      <div id="wiz-rf-later" class="wizard-info-box" style="display:none">
        <p>You can configure additional RF devices later via the <strong>RF Devices</strong> menu.</p>
      </div>
    </fieldset>
  </form>
