...
void sendI2CPWMinit()
{
  // #365 ROOT-CAUSE FIX: commit 1efa17e (Nov 2024, "Refactor RF code +
  // first try at virtual CO2 sensor") accidentally changed bytes 8/10
  // from 0x09/0x09 (the correct PWM-init payload) to 0x13/0x13 (the
  // CO2-init payload). The CVE ECO 2 SP unit needs 0x09/0x09 to enter
  // PWM-accept mode. With 0x13/0x13 the unit ignores PWM frames on
  // cold boot. Reverted to the pre-1efa17e payload.
