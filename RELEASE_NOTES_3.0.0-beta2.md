...
### New in beta2 (since beta1)

- Fix: coredump download (/getcoredump) crashed the device due to use-after-free — lambda captured local stack buffer by reference, read into wrong buffer, no null check on partition
- Fix: WebSocket buffer race condition — concurrent clients could corrupt the shared message buffer causing heap corruption and device crashes. Added mutex synchronization and proper cleanup on disconnect
- Fix: OTA static buffer overflow — buffer was never cleared between OTA operations, eventually overflowing on repeated firmware updates
- Fix: I2C safe guard null pointer dereference — cast without null check before use
- Fix: uninitialized decrypt buffer — could contain garbage data if decryption fails
- Fix: all strncpy replaced with strlcpy for guaranteed null-termination
- Fix: unnecessary static keyword removed from local variables
- Added OpenAPI spec as standalone file (docs/openapi.json) for developers
...
https://github.com/arjenhiemstra/ithowifi/releases/download/Version-3.0.0-beta2/nrgitho-v3.0.0-beta2.bin
